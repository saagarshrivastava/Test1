...
import com.mycompany.test1.config.KafkaProperties;
...
class Test1KafkaResourceIT {
    ...
    void setup() {
        ...
        Test1KafkaResource kafkaResource = new Test1KafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/test-1-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/test-1-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
